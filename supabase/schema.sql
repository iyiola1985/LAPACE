...
begin
  ...
  if lower(new.email) in ('lapaceltd@gmail.com', 'admin@lapacealuminium.com') then
    resolved_role := 'admin';
  end if;
...
create or replace function public.force_reserved_admin()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists profiles_force_admin on public.profiles;
create trigger profiles_force_admin
  before insert or update on public.profiles
  for each row execute function public.force_reserved_admin();
